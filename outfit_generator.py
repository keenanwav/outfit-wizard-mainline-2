...
def delete_file_batch(file_batch: List[str]) -> None:
...
def cleanup_merged_outfits():
    """Clean up old unsaved outfit files"""
    try:
        ...
        if files_to_delete:
            with ThreadPoolExecutor(max_workers=settings['max_workers']) as executor:
                ...
                # Submit batch jobs and process them
                for batch in batches:
                    executor.submit(delete_file_batch, batch)
